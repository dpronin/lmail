    ...
    void _exec_()
    {
        ...
        if (auto users = (*storage_)->get_all<User>(where(c(&User::username) == username)); (b_success = !users.empty())) {
            ...
            if (b_success) {
                ...
                auto new_state        = std::make_shared<CmdState>(
                    cmds_names_t{cmds_names.begin(), cmds_names.end()},
                    colorize::cbrown("lmail (") + make_user_name_cgreen(*logged_user) + colorize::cbrown(") > "));
